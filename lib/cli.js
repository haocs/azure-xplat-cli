...
_.extend(AzureCli.prototype, {
  initSetup: function () {
    ...
    if (!self.debug && process.listeners('uncaughtException').length === 0) {
      self.uncaughExceptionHandler = function (err) {
		// If we reach this point, clearProgress() won't work.
		// Use process.exit() force exit.
  ...
  recordError: function (err) {
    if (err) {
      ...
      log.writeCapturedSillyLogs(self.getSillyErrorFile(), process.env.AZURE_CLI_APPEND_LOGS);
    }
  },
  ...
  exit: function (level, message, exitCode) {
    ...
    if (log.shouldWaitForStdoutDrained) {
      process.on('exit', function () { process.exit(exitCode); });
    }
	// Don't call process.exit on success.
    else if (exitCode) {
